          ...
          <DialogHeader>
           ...
           <ScrollArea className="h-[50vh] my-4">
                <div className="text-sm leading-relaxed whitespace-pre-wrap font-mono p-4">
                  {currentCall?.transcript.split(/Speaker\d:|Mrs\. Mehta:|Aarav \(Scammer\):|Rohit \(Scammer\):|Emily \(Scammer\):|Rishab:|Rahul:|Neha:/).map((part, index) => {
                      if (index === 0) return part.trim() ? <p key={index}>{part.trim()}</p> : null;
                      
                      const fullMatch = currentCall?.transcript.match(/(Speaker\d:|Mrs\. Mehta:|Aarav \(Scammer\):|Rohit \(Scammer\):|Emily \(Scammer\):|Rishab:|Rahul:|Neha:)/g);
                      const speakerLabel = fullMatch ? fullMatch[index - 1] : '';

                      let speakerName = 'Unknown';
                      let isCaller = false;
                      if (speakerLabel.startsWith('Speaker1') || speakerLabel.includes('(Scammer)') || speakerLabel.startsWith('Rishab') || speakerLabel.startsWith('Rahul')) {
