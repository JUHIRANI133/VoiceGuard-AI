...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchUploadedFiles = useCallback(async () => {
    try {
        ...
        setState(prevState => ({ ...prevState, uploadedFiles: files.sort((a, b) => b.id.localeCompare(a.id)) }));
    } catch (error) {
  ...
  const uploadAudioFile = async (file: File) => {
    ...
    try {
        ...
        audio.onerror = (e) => {
            ...
            toast({ title: "Upload Error", description: "Could not process audio file metadata.", variant: "destructive" });
        }
    } catch (error) {
  ...
  const deleteUploadedFile = async (id: string) => {
      ...
      try {
          ...
          // Also refresh state in case it's a sync issue
          setState(prevState => ({
              ...prevState,
              uploadedFiles: prevState.uploadedFiles.filter(f => f.id !== id),
          }));
